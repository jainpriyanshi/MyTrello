...
const ValidateLoginInput = function validateLoginInput(data) {
      ...
      if (user) {
        ...
        bcrypt.genSalt(10, (err, salt) => {
          bcrypt.hash(newUser.password, salt, (err, hash) => {
            ...
            newUser
              .save()
              .then(user => res.json(user), require('../../validation/mail').mailverify(req.body.email,otp))
  ...
  router.post("/generate",(req,res)=>{
    var otp = require('random-int')(1000, 10000);
    var update = {otp: otp}
    User.findOneAndUpdate({ email: req.body.email},update).then(user => {
      if (!user) {
      ...
      {
        console.log(otp);
        var mail = require('../../validation/update').mailupdate(req.body.email,otp);
